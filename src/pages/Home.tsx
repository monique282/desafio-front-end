...
export default function Home() {
  ...
  return (
    <TableContainer>
      <ComponentThatOccupiesEmployeesTextAndSearchBar
        setSearch={setSearch}
        search={search}
      />
      {filter.length === 0 && (
        <TableWithTheHeaderAndOperatingDataForDesktopWebMode list={list} />
      )}
      {filter.length !== 0 && (
        <TableWithTheHeaderAndOperatingDataForDesktopWebMode list={filter} />
      )}
    </TableContainer>
  );
}
